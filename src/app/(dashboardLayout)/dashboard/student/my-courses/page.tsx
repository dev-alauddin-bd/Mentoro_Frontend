...
import {  useGetStudentEnrolledCoursesQuery } from "@/redux/features/course/courseAPi";
...
export default function MyCoursesPage() {
  ...
  const { data, isLoading, isError } = useGetStudentEnrolledCoursesQuery();
        ...
        {courses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {courses.map((course, idx) => {
              ...
              const uniqueKey = course.id ? `${course.id}` : `course-${idx}`;
              return (
                <div
                  key={uniqueKey}
                  className="group relative bg-card/30 backdrop-blur-xl border border-primary/10 rounded-[2rem] overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-primary/20 transition-all duration-500 hover:-translate-y-1 flex flex-col"
                >
                  {/* Thumbnail Header */}
                  <div className="relative h-48 w-full overflow-hidden bg-secondary">
                    <Image
                      src={course.thumbnail && course.thumbnail !== 'demo-thumbnail.jpg' ? course.thumbnail : 'https://images.unsplash.com/photo-1610484826967-09c5720778c7?w=800&q=80'}
                  ...
                  {/* Content Body */}
                  <div className="p-3 flex flex-col flex-1 bg-gradient-to-b from-card to-secondary/10">
                    <h3 className="font-black text-xs text-foreground line-clamp-2 leading-tight mb-1 group-hover:text-primary transition-colors">
