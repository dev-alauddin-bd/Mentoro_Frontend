...
} from "@/redux/features/enroll/enrollApi";
import {
...
export default function CoursePlayerPage() {
  ...
  // -------------------------------------------------------------------------
  // Load enrolled courses first (to map slug → course ID)
  // -------------------------------------------------------------------------
  const {
  ...
  } = useGetStudentEnrolledCoursesQuery(undefined);

  // Find the course that matches the slug from the URL
  const selectedCourse = enrolledCoursesResponse?.data?.find(
    (c: any) => c.slug === slug
  );

  // Load modules for the resolved course ID
  const {
    data: courseResponse,
    isLoading: isModulesLoading,
    isError: isModulesError,
    refetch: refetchModules,
  } = useGetStudentCourseModulesQuery(selectedCourse?.slug, {
    skip: !selectedCourse?.slug,
  });
  ...
  // Helper values
  // -------------------------------------------------------------------------
  const isLoading = isEnrollLoading || isModulesLoading;
  const isError = isEnrollError || isModulesError;
  const refetch = () => {
    refetchEnroll();
    refetchModules();
  };
  ...
  useEffect(() => {
    if (navItems.length > 0 && !activeItem) {
      const idx = navItems.findIndex(
        (n) => n.type === "lesson" && !n.data.isCompleted && n.data.isUnlocked
      );
  ...
  // -------------------------------------------------------------------------
  // Reset assignment state when active item changes
  // -------------------------------------------------------------------------
  useEffect(() => {
  ...
  const goNextAndComplete = async () => {
    ...
    // Move to next item or finish course
  ...
  const handleSubmitAssignment = async () => {
    ...
    try {
      ...
      toast.error(
        err?.data?.message || t("student.player.failed_submit_toast")
      );
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-1000 pb-20">
      ...
      <div className="w-full lg:w-96 border-r flex flex-col bg-card overflow-hidden">
        <div className="p-5 border-b bg-card z-10 shrink-0">
          <Link
            href="/dashboard/student/my-courses"
            className="text-xs flex items-center text-muted-foreground hover:text-primary mb-3 transition-colors"
          >
          ...
          <div className="mt-3">
            <div className="flex justify-between text-xs mb-1 font-medium">
              ...
              <span className="text-primary">
                {(() => {
                  const total =
                    course.modules?.reduce((a: number, m: any) => a + m.lessonCount, 0) ||
                    1;
                  const done =
                    course.modules?.reduce((a: number, m: any) => a + m.completedCount, 0) ||
                    0;
              ...
              >
                ...
                {isExpanded && (
                  <div className="p-2 space-y-0.5">
                    ...
                    {module.lessons?.map((lesson: any) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={`truncate ${isActive ? "text-primary font-semibold" : "text-foreground"
                            }`}>{lesson.title}</span>
                          <span className="ml-auto text-[10px] text-muted-foreground whitespace-nowrap">
                            {lesson.duration}
                            {t("student.player.mins")}
                          </span>
                        </button>
                      );
                    })}
                    ...
                    {module.quiz && (() => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => goToItem(navIdx)}
                          className={`w-full flex items-center gap-2.5 p-2.5 rounded-lg text-left transition-all text-sm mt-1 ${isActive
                    ...
                    {module.assignment && (() => {
                      const navIdx = navItems.findIndex(
                        (n) => n.id === module.assignment.id
                      );
                      const isActive = activeIndex === navIdx;
                      return (
                        <button
                          key={`asgn-${module.assignment.id}`}
                          onClick={() => goToItem(navIdx)}
                          className={`w-full flex items-center gap-2.5 p-2.5 rounded-lg text-left transition-all text-sm mt-0.5 ${isActive
        ...
        {isCourseCompletedMode ? (
          <div className="flex flex-col flex-1 items-center justify-center text-center p-10 bg-background">
            ...
            <h3 className="text-3xl font-black text-foreground mb-4">
              {t("student.player.course_completed")}
            </h3>
              ...
              <div className="w-full flex flex-col">
                ...
                <div className="max-w-4xl w-full mx-auto p-6 md:p-10">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-border">
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-3">
                        <span className="px-3 py-1 bg-secondary text-xs font-bold uppercase tracking-widest rounded-full">
                          {activeItem.data.duration} {t("student.player.mins")}
                        </span>
                  ...
                  {activeItem.data.description && (
                    <p className="mt-4 text-foreground">{activeItem.data.description}</p>
                  )}
              ...
              <div className="max-w-3xl w-full mx-auto p-6 md:p-10 space-y-8">
                <div className="flex items-center gap-4 bg-purple-500/10 p-4 rounded-2xl border border-purple-500/20">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-purple-500 font-bold uppercase tracking-widest">
                      {t("student.player.practical_task")}
                    </p>
                  </div>
                </div>
                ...
                {assignmentSubmitted ? (
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-8 text-center space-y-3">
                    ...
                    <h3 className="text-xl font-bold text-foreground">
                      {t("student.player.submitted")}
                    </h3>
                    <p className="text-muted-foreground">
                      {t("student.player.review_soon")}
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <textarea
                      ...
                      placeholder={
                        activeItem.data.submissionType === "link"
                          ? t("student.player.placeholder_link")
                          : t("student.player.placeholder_text")
                      }
                    ...
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 bg-purple-500/10 text-purple-600 rounded-xl font-bold text-sm hover:bg-purple-500 hover:text-white transition-all border border-purple-500/20 shadow-lg shadow-purple-500/5"
