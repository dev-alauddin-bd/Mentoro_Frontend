...
import { Scale, Gavel, Book, FileCheck, ArrowDown, Zap, Loader2 } from "lucide-react";
import { useGetLegalDocumentBySlugQuery } from "@/redux/features/legal/legalApi";

export default function TermsOfServicePage() {
  const { t } = useTranslation();
  const { data: legalData, isLoading } = useGetLegalDocumentBySlugQuery("terms-of-service");

  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-background">
        <Loader2 className="w-12 h-12 animate-spin text-primary" />
        <p className="text-xs font-black uppercase tracking-widest text-muted-foreground animate-pulse">Retrieving Terms...</p>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-foreground leading-[0.9]">
              {dbData?.title || t("terms_of_service.title_start")} <br />
              {!dbData && <span className="text-primary italic font-serif">{t("terms_of_service.title_end")}</span>}
            </h1>
