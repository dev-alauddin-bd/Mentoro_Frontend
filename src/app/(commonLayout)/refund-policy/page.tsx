...
import { RotateCcw, ShieldCheck, Clock, Percent, Mail, ArrowDown, Loader2 } from "lucide-react";
import { useGetLegalDocumentBySlugQuery } from "@/redux/features/legal/legalApi";

export default function RefundPolicyPage() {
  const { t } = useTranslation();
  const { data: legalData, isLoading } = useGetLegalDocumentBySlugQuery("refund-policy");
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-foreground leading-[0.9]">
              {dbData?.title || t("refund_policy.title_start")} <br />
              {!dbData && <span className="text-primary italic font-serif">{t("refund_policy.title_end")}</span>}
            </h1>
